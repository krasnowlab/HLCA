...
load_tissue_facs = function(tissue_of_interest){
  ...
  filename = here('patient_stats', '3.csv')
  star.data = as.data.frame(t(read.csv(filename, sep=",", row.names=1)))
  
  # Create the Seurat object with all the data
  tiss <- CreateSeuratObject(raw.data = raw.data, project = "Human Lung SS2 - Patient 3")
...
load_tissue_droplet = function(tissue_of_interest){
  ...
  # Create the Seurat object with all the data
  tiss <- CreateSeuratObject(raw.data = raw.data, project = "Human Lung 10x - Patient 3")
  tiss <- AddMetaData(object = tiss, meta.data)
