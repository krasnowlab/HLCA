...
load_tissue_facs = function(tissue_of_interest){
  ...
  plate_metadata_filename = here::here('metadata', 'SS2.csv')
  ...
  filename = here::here('datass2', paste0(tissue_of_interest, '-counts.csv'))
  ...
  filename = here('patient_stats', '1.csv')
  ...
  if (tissue_of_interest == 'Lung' || tissue_of_interest == 'all') {
    LungCells <- rownames(tiss@meta.data[which(tiss@meta.data[,'tissue'] == 'Lung'),])
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Normal'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) > 11),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'ssff'),]))))),'gating'] = 'CD45+ Epcam-'
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Normal'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) < 12),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'ssff'),]))))),'gating'] = 'CD45- Epcam-'
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Tumor'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) < 9),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'ssff'),]))))),'gating'] = 'CD45- Epcam-'
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Tumor'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) > 8),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'ssff'),]))))),'gating'] = 'CD45+ Epcam-'
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Normal'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) > 11),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'biohub'),]))))),'gating'] = 'CD45+ Epcam(+/-)'
    tiss@meta.data[intersect(LungCells,intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'region'] == 'Normal'),]),intersect(rownames(tiss@meta.data[which(as.numeric(lapply(tiss@meta.data[,'cell.id'], function(x) as.numeric(gsub('[A-Z]{1}([0-9]{1,2})_B(.*)', "\\1", x)))) < 12),]),intersect(rownames(tiss@meta.data[which(tiss@meta.data[,'label'] == 'Epcam (-)'),]),rownames(tiss@meta.data[which(tiss@meta.data[,'sorter'] == 'biohub'),]))))),'gating'] = 'CD45- Epcam(+/-)'
    tiss@meta.data[intersect(LungCells,rownames(tiss@meta.data[which(is.na(tiss@meta.data[,'gating'])),])),'gating'] = 'CD45- Epcam+'
  
  }
  
  if (tissue_of_interest == 'Blood' || tissue_of_interest == 'all') {
    BloodCells <- rownames(tiss@meta.data[which(tiss@meta.data[,'tissue'] == 'Blood'),])
    gating <- unlist(lapply(BloodCells, function(x) strsplit(x, "\\.")[[1]][1]))
    gating <- lapply(gating, function(x) strsplit(x, "_")[[1]][3])
    gating <- as.character(gating)
    gating[grep('homo', gating)] <- 'unknown'
    tiss@meta.data[BloodCells,'gating'] = gating
  }
...
load_tissue_droplet = function(tissue_of_interest){
  ...
  droplet_metadata_filename = here::here('metadata', '10X.csv')
  ...
  if (tissue_of_interest != 'all') {
    tissue_metadata = filter(droplet_metadata, tissue == tissue_of_interest)[,c('channel','tissue','region','compartment','area','patient')]
  } else {
    tissue_metadata = droplet_metadata[,c('channel','tissue','region','area','compartment')]
  }
  
  subfolder = tissue_metadata$channel[1]
  raw.data <- Read10X(data.dir = here::here('data10x', subfolder))
  ...
  if (length(tissue_metadata$channel) > 1){
    ...
    for(i in 2:nrow(tissue_metadata)){
      ...
      new.data <- Read10X(data.dir = here::here('data10x', subfolder))
