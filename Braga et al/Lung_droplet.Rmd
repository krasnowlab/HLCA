...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x, do.return = TRUE, group.by = "orig.ident")
TSNEPlot(object = tiss10x, do.return = TRUE, group.by = "location")
TSNEPlot(object = tiss10x, do.return = TRUE, group.by = "celltype")
TSNEPlot(object = tiss10x, do.return = TRUE, group.by = "ID")
TSNEPlot(object = tiss10x, do.return = TRUE, group.by = "GEO_Sample")
...
```{r, fig.width=10, fig.height=10}
cells_to_use <- WhichCells(tiss10x, ident = c(13,18))
...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.stromal, do.return = TRUE, group.by = "orig.ident")
TSNEPlot(object = tiss10x.stromal, do.return = TRUE, group.by = "location")
TSNEPlot(object = tiss10x.stromal, do.return = TRUE, group.by = "celltype")
TSNEPlot(object = tiss10x.stromal, do.return = TRUE, group.by = "ID")
TSNEPlot(object = tiss10x.stromal, do.return = TRUE, group.by = "GEO_Sample")
```

```{r, fig.height=50, fig.width=10}

genes_to_check = c('CLDN5', 'EPCAM', 'COL1A2', 'PTPRC', 'SERPINF1', 'SPINT2', 'SFRP2', 'FGFR4', 'ITGA8', 'GPC3', 'ASPN', 'FGF18', 'HIGD1B', 'KCNK3', 'ACTA2', 'TAGLN', 'ADAMTS4', 'PDGFRA', 'PDGFRB', 'MSLN', 'UPK3B')
...
```{r, fig.width=10, fig.height=10}
tiss10x.epi = SubsetData(tiss10x, ident.use = c(3,27,15,23,1,6,15,5,8,26,17))
```
...
tiss10x.epi <- tiss10x.epi %>% FindClusters(reduction.type = "pca", dims.use = normal.pcs,
    resolution = normal.res.use, print.output = 0, save.SNN = TRUE, plot.SNN = TRUE, force.recalc = TRUE) %>%
...
TSNEPlot(object = tiss10x.epi, do.return = TRUE, group.by = "location")
TSNEPlot(object = tiss10x.epi, do.return = TRUE, group.by = "celltype")
TSNEPlot(object = tiss10x.epi, do.return = TRUE, group.by = "ID")
TSNEPlot(object = tiss10x.epi, do.return = TRUE, group.by = "GEO_Sample")
```

```{r, fig.height=50, fig.width=10}

genes_to_check = c('CLDN5', 'EPCAM', 'COL1A2', 'PTPRC', 'MUC5B', 'SFTPC', 'SFTPB', 'ETV5', 'PDPN', 'CAV1', 'AGER', 'MYRF', 'CLIC5', 'FOXJ1', 'C20orf85', 'TUBB4B', 'SCGB1A1', 'SCGB3A2', 'WIF1', 'HHIP', 'SERPINA1', 'KRT5', 'TP53', 'S100A8', 'S100A9', 'S100A12', 'CD14', 'FCGR3B')
...
```{r, fig.width=10, fig.height=10}
cells_to_use <- WhichCells(tiss10x.epi, ident = c(2,8,11,4))
cells_to_use <- intersect(intersect(cells_to_use, names(which(tiss10x.epi@data['PTPRC',] == 0))), names(which(tiss10x.epi.r2@data['S100A8',] < 3)))
tiss10x.epi.r2 = SubsetData(tiss10x.epi, cells.use = cells_to_use)
```

```{r, fig.width=10, fig.height=10}
tiss10x.epi.r2 <- tiss10x.epi.r2 %>% ScaleData() %>%
  FindVariableGenes(do.plot = TRUE, x.high.cutoff = Inf, y.cutoff = 0.5, x.low.cutoff = 0.25) %>%
  RunPCA(do.print = FALSE) %>% ProjectPCA(do.print = FALSE)
```
...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.epi.r2, do.return = TRUE, group.by = "orig.ident", pt.size = 2)
TSNEPlot(object = tiss10x.epi.r2, do.return = TRUE, group.by = "location", pt.size = 2)
TSNEPlot(object = tiss10x.epi.r2, do.return = TRUE, group.by = "celltype", pt.size = 2)
TSNEPlot(object = tiss10x.epi.r2, do.return = TRUE, group.by = "ID", pt.size = 2)
TSNEPlot(object = tiss10x.epi.r2, do.return = TRUE, group.by = "GEO_Sample", pt.size = 2)
```

```{r, fig.height=50, fig.width=10}

genes_to_check = c('CLDN5', 'EPCAM', 'COL1A2', 'PTPRC', 'MUC5B', 'SFTPC', 'SFTPB', 'ETV5', 'PDPN', 'CAV1', 'AGER', 'MYRF', 'CLIC5', 'FOXJ1', 'C20orf85', 'TUBB4B', 'SCGB1A1', 'SCGB3A2', 'WIF1', 'HHIP', 'SERPINA1', 'KRT5', 'KRT17', 'KRT15', 'S100A8', 'S100A9', 'S100A12', 'CD14', 'FCGR3B', 'MKI67')
...
FeaturePlot(tiss10x.epi.r2, genes_to_check$gene, pt.size = 2, nCol = 2, cols.use = my.cols, dark.theme = TRUE)
```
...
```{r, fig.width=10, fig.height=10}
cells_to_use = WhichCells(tiss10x, ident = c(14,20))
...
TSNEPlot(object = tiss10x.endo, do.label = T, pt.size = 3, label.size = 10)

```

```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.endo, do.return = TRUE, group.by = "orig.ident", pt.size = 2)
TSNEPlot(object = tiss10x.endo, do.return = TRUE, group.by = "location", pt.size = 2)
TSNEPlot(object = tiss10x.endo, do.return = TRUE, group.by = "celltype", pt.size = 2)
TSNEPlot(object = tiss10x.endo, do.return = TRUE, group.by = "ID", pt.size = 2)
TSNEPlot(object = tiss10x.endo, do.return = TRUE, group.by = "GEO_Sample", pt.size = 2)
```

```{r, fig.height=45, fig.width=10}

genes_to_check = c('CLDN5', 'EPCAM', 'COL1A2', 'PTPRC', 'GJA5', 'PTGIS', 'EDNRB', 'CA4', 'IL7R', 'MYC', 'SPRY1', 'SOX17', 'PROX1', 'CCL21')
...
FeaturePlot(tiss10x.endo, genes_to_check$gene, pt.size = 3, nCol = 2, cols.use = my.cols, dark.theme = TRUE)
```
...
```{r, fig.width=10, fig.height=10}
tiss10x.immune = SubsetData(tiss10x, ident.use = c(9,11,16,24,12,19,4,7,0,2,25,22,21,10))
```
...
tiss10x.immune <- tiss10x.immune %>% FindClusters(reduction.type = "pca", dims.use = normal.pcs,
    resolution = normal.res.use, print.output = 0, save.SNN = TRUE, plot.SNN = TRUE, force.recalc = TRUE, k.param = 20, prune.SNN = 1/10) %>%
    RunTSNE(dims.use = normal.pcs, seed.use = 10, perplexity=30)
...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.immune, do.return = TRUE, group.by = "orig.ident", pt.size = 2)
TSNEPlot(object = tiss10x.immune, do.return = TRUE, group.by = "location", pt.size = 2)
TSNEPlot(object = tiss10x.immune, do.return = TRUE, group.by = "celltype", pt.size = 2)
TSNEPlot(object = tiss10x.immune, do.return = TRUE, group.by = "ID", pt.size = 2)
TSNEPlot(object = tiss10x.immune, do.return = TRUE, group.by = "GEO_Sample", pt.size = 2)
```

```{r, fig.height=120, fig.width=10}

genes_to_check = c('PTPRC', 'CLDN5', 'EPCAM', 'COL1A1', 'CD3E', 'CD4', 'CD8A', 'GZMH', 'GZMB', 'GZMK', 'CCL5', 'GZMA', 'CD40LG', 'LDHB', 'LEF1', 'CCR7', 'KLRB1', 'TYROBP', 'NKG7', 'FCER1G', 'CD79A', 'MS4A1', 'JSRP1', 'FCGR3B', 'MMP9', 'S100A8', 'MS4A2', 'CPA3', 'RGS13', 'SIGLEC8', 'MARCO', 'MSR1', 'MRC1', 'C1QA', 'LILRB4', 'IRF8', 'CLEC4C', 'SMPD3', 'GPR183', 'HLA-DPB1', 'CLEC9A', 'LAMP3', 'CD1C', 'PKIB', 'F13A1', 'EREG', 'CLEC5A', 'TREM2', 'CD14', 'VCAN', 'FCGR3A', 'CD300E', 'MKI67', 'TOP2A')
...
```{r, fig.width=10, fig.height=10}
tiss10x.immune.r2 = SubsetData(tiss10x.immune, ident.use = c(3,5,8))
```
...
tiss10x.immune.r2 <- tiss10x.immune.r2 %>% FindClusters(reduction.type = "pca", dims.use = 1:normal.n.pcs,
    resolution = normal.res.use, print.output = 0, save.SNN = TRUE, force.recalc = TRUE, k.param = 15, plot.SNN = TRUE) %>%
    RunTSNE(dims.use = 1:normal.n.pcs, seed.use = 10, perplexity=30)
...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.immune.r2, do.return = TRUE, group.by = "orig.ident", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r2, do.return = TRUE, group.by = "location", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r2, do.return = TRUE, group.by = "celltype", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r2, do.return = TRUE, group.by = "ID", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r2, do.return = TRUE, group.by = "GEO_Sample", pt.size = 2)
```
...
FeaturePlot(tiss10x.immune.r2, genes_to_check, pt.size = 1, nCol = 2, cols.use = my.cols, dark.theme = TRUE)
...
```{r, fig.width=10, fig.height=10}
tiss10x.immune.r3 = SubsetData(tiss10x.immune, ident.use = c(2,6,10,7))
```
...
```{r, fig.width=10, fig.height=10}
TSNEPlot(object = tiss10x.immune.r3, do.return = TRUE, group.by = "orig.ident", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r3, do.return = TRUE, group.by = "location", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r3, do.return = TRUE, group.by = "celltype", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r3, do.return = TRUE, group.by = "ID", pt.size = 2)
TSNEPlot(object = tiss10x.immune.r3, do.return = TRUE, group.by = "GEO_Sample", pt.size = 2)
```
...
FeaturePlot(tiss10x.immune.r3, genes_to_check, pt.size = 1, nCol = 2, cols.use = my.cols, dark.theme = TRUE)
...
# Save the object
```{r, fig.width=10, fig.height=10}
save(tiss10x.anno, file = here('seurat', 'dropseq_barga_normal_lung_seurat_tiss10x.anno.20200122.RC4.Robj'))
```
